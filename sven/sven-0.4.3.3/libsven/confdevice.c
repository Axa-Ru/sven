...
static GtkItemFactoryEntry menu_items[] =
{
 // { "/New simulete key",NULL,new_key_factory_cb,0,"<Item>" },
  ...
  { "/Window Utils",		NULL,	0,0,"<Branch>"},
  { "/Window Utils/Close document",	NULL,new_key_factory_cb,8,"<Item>"},
  { "/Window Utils/Quit",	NULL,new_key_factory_cb,9,"<Item>"},
  { "/Window Utils/Close window",	NULL,new_key_factory_cb,10,"<Item>"},
  { "/Window Utils/App Sw",	NULL,new_key_factory_cb,11,"<Item>"},
  
  { "/Mix",		NULL,	0,0,"<Branch>"},
  { "/Mix/Volume Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Volume Down",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Volume Mute",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Bass Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Bass Down",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Treble Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Treble Down",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Pcm Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Pcm Down",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Speaker Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Speaker Down",NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Line Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Line Down",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Mic Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Mic Down",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Cd Up",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Mix/Cd Down",	NULL,gtk_ifactory_cb,0,"<Item>"},

  { "/CD",		NULL,	0,0, "<Branch>" },
  { "/CD/Play",		NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/CD/Next",		NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/CD/Pause",	NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/CD/Prev",		NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/CD/Stop",		NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/CD/Eject",	NULL,gtk_ifactory_cb,0,"<Item>"},
  
  { "/Other",		NULL,	0,0,"<Branch>" },
  { "/Other/Clipboard Manager",NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Other/Convert",NULL,gtk_ifactory_cb,0,"<Item>"},
  { "/Other/Convert Case",NULL,gtk_ifactory_cb,0,"<Item>"},
};
...
GtkWidget *get_option_menu(Sconfdevice *scd)
{
   ...
   option_menu = gtk_item_factory_get_widget(item_factory, "<sven>");
	...
	for (l = scd->sven->plugins; l; l = l->next)
	{
		SvenPlugin *plugin = (SvenPlugin *)l->data;
		plug_func=plugin->add_function_list();
		...
		for (ll = plug_func; ll; ll = ll->next)
		{
			...
			gtk_item_factory_create_item(item_factory, &entry, (gpointer) g_basename(plugin->filename) , 1);
...
static void add_print_list(GtkTreeView *list_tree_view,Sconfdevice *scd)
{
	...
	if(scd->device==0)
	{
		if(scd->sven->cfgfile!=NULL)
		{
			list = scd->sven->cfgfile->sections;
			while (list)
			{
				int key_code=0;
				int enable=0;
				...
				sven_cfg_read_int(scd->sven->cfgfile,section->name, "enable",&enable);
				sven_cfg_read_string(scd->sven->cfgfile,section->name, "type",&type);
				sven_cfg_read_string(scd->sven->cfgfile,section->name, "description",&description);
				sven_cfg_read_string(scd->sven->cfgfile,section->name, "command",&command);
				sven_cfg_read_string(scd->sven->cfgfile,section->name, "comment",&comment);
				...
				else if(g_strncasecmp (g_strndup(&type[strlen(type)-strlen(so)], strlen(so)),so,3)==0)
				{
					...
					if(scd->sven->mix->enable_plugin==1)
					{
						splg = g_slist_nth(scd->sven->plugins,plugin_find_pos(scd->sven,type))->data;
					...
					else if (scd->sven->mix->enable_plugin==0)
					{
						print_key_list (scd->list_model,&iter,description,key,command,comment,g_strdup(g_basename(type)),enable);
					}
					else
				...
				{
					printf(RED "Bad config line key config\n");
				}
	...
	{
		if(scd->sven->cfgfilemouse!=NULL)
		{
			list = scd->sven->cfgfilemouse->sections;
			while (list)
			{
				int but_code=0;
				int enable=0;
				...
				gchar **but_stat = g_strsplit(section->name,":",2);
				but_code=atoi(but_stat[0]);
				if(but_stat[1]==NULL)
					stat=0;
				else
					stat=atoi(but_stat[1]);
				DEBUG_MSG("READ:%s\n",section->name);
	
				sven_cfg_read_int(scd->sven->cfgfilemouse,section->name, "enable",&enable);
				sven_cfg_read_string(scd->sven->cfgfilemouse,section->name, "type",&type);
				sven_cfg_read_string(scd->sven->cfgfilemouse,section->name, "description",&description);
				sven_cfg_read_string(scd->sven->cfgfilemouse,section->name, "command",&command);
				sven_cfg_read_string(scd->sven->cfgfilemouse,section->name, "comment",&comment);
				...
				if(g_strcasecmp (type,def)==0)
				{
					...
					but=g_strdup_printf ("%s%s%d",str,str[0] ? "+" : "",but_code);
					print_key_list (scd->list_model, &iter,description,but,command, comment,def,enable);
				}
				else if(g_strcasecmp (type,cmd)==0)
				{
					model = gtk_tree_view_get_model (list_tree_view);
					...
					ModifierToStringMouse (stat, str);
					but=g_strdup_printf ("%s%s%d",str,str[0] ? "+" : "",but_code);
					print_key_list (scd->list_model, &iter,description,but,command,comment,cmd,enable);
				}
				else if(g_strcasecmp (type,"sem")==0)
				{
					model = gtk_tree_view_get_model (list_tree_view);
					...
					ModifierToStringMouse (stat, str);
					but=g_strdup_printf ("%s%s%d",str,str[0] ? "+" : "",but_code);
					print_key_list (scd->list_model, &iter,description,but,command,comment,"sem",enable);
				}
				else if(g_strncasecmp (g_strndup(&type[strlen(type)-strlen(so)], strlen(so)),so,3)==0)
				{
					model = gtk_tree_view_get_model (list_tree_view);
					...
					if(scd->sven->mix->enable_plugin==1)
					{
						...
						if(splg!=NULL)
						{
							print_key_list (scd->list_model, &iter,description,but,command,comment,g_strdup(g_basename(type)),enable);
						}
					}
					else if (scd->sven->mix->enable_plugin==0)
					{
				...
				{
					printf(RED "Bad config line mouse config\n");
				}
...
GtkWidget* sconfdevice_new (Sven *sven,gchar *name,int device)
{
	...
	DEBUG_MSG("Start create_list_gui\n");
...
static void file_dialog_window (GtkButton * button, Sconfdevice *scd)
{
	...
	scd->filechooser = gtk_file_chooser_dialog_new ("", NULL, GTK_FILE_CHOOSER_ACTION_OPEN, NULL);
...
void sconfdevice_save (Sconfdevice *scd)
{
	...
	if(scd->device==0)
	{
		...
		if (gtk_tree_model_get_iter (model, &iter, path) == FALSE)
		{
			...
			scd->sven->cfgfile = sven_cfg_open_file(filename);
			if (!scd->sven->cfgfile)
				scd->sven->cfgfile = sven_cfg_new();
			DEBUG_MSG("gtk_tree_model_get_iter\n");
			return;
		}
		...
		do
		{
			...
			sven_cfg_write_string	(cfgfile,g_strdup_printf("%d:%d",key,mod), "type",info);
			sven_cfg_write_int 	(cfgfile,g_strdup_printf("%d:%d",key,mod), "enable",enable);
			sven_cfg_write_string 	(cfgfile,g_strdup_printf("%d:%d",key,mod), "description",description);
			sven_cfg_write_string 	(cfgfile,g_strdup_printf("%d:%d",key,mod), "command", command);
			sven_cfg_write_string 	(cfgfile,g_strdup_printf("%d:%d",key,mod), "comment", text_disp);
	...
	else if(scd->device==1)
	{
		...
		if (gtk_tree_model_get_iter (model, &iter, path) == FALSE)
		{
			...
			sven_cfg_free_file(filename);
			scd->sven->cfgfilemouse = sven_cfg_open_file(filename);
			if (!scd->sven->cfgfilemouse)
				scd->sven->cfgfilemouse = sven_cfg_new();
			DEBUG_MSG("gtk_tree_model_get_iter\n");
			return;
		}
		...
		do
		{
			...
			sven_cfg_write_string	(cfgfile,g_strdup_printf("%d:%d",but,mod), "type",info);
			sven_cfg_write_int 	(cfgfile,g_strdup_printf("%d:%d",but,mod), "enable",enable);
			sven_cfg_write_string 	(cfgfile,g_strdup_printf("%d:%d",but,mod), "description",description);
			sven_cfg_write_string 	(cfgfile,g_strdup_printf("%d:%d",but,mod), "command", command);
			sven_cfg_write_string 	(cfgfile,g_strdup_printf("%d:%d",but,mod), "comment", text_disp);
