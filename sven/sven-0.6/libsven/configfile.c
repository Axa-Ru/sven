 ...
 * configfile.c - the read write config file
 *
...
ConfigFile *sven_cfg_open_file(gchar * filename)
{
	...
	while (lines[i])
	{
		if (lines[i][0] == '[')
		{
			...
			{
				*tmp = '\0';
				section = sven_cfg_create_section(cfg, &lines[i][1]);
			}
		}
		else if (lines[i][0] != '#' && section)
		{
			if ((tmp = strchr(lines[i], '=')))
			{
				*tmp = '\0';
				tmp++;
				sven_cfg_create_string(section, lines[i], tmp);
			}
		}
		i++;
	}
...
gchar * sven_cfg_get_default_filename(void)
{
...
ConfigFile * sven_cfg_open_default_file(void)
{
	...
	ret = sven_cfg_open_file(sven_cfg_get_default_filename());
	if (!ret)
		ret = sven_cfg_new();
	return ret;
}
...
{
	return sven_cfg_write_file(cfg, sven_cfg_get_default_filename());
}
...
gboolean sven_cfg_read_string(ConfigFile * cfg, gchar * section, gchar * key, gchar ** value)
{
	...
	if (!(sect = sven_cfg_find_section(cfg, section)))
	return FALSE;
...
gboolean sven_cfg_read_int(ConfigFile * cfg, gchar * section, gchar * key, gint * value)
{
	...
	if (!sven_cfg_read_string(cfg, section, key, &str))
		return FALSE;
	*value = atoi(str);
	g_free(str);
...
void sven_cfg_write_int(ConfigFile * cfg, gchar * section, gchar * key, gint value)
{
	...
	sven_cfg_write_string(cfg, section, key, strvalue);
	g_free(strvalue);
}
...
void sven_cfg_write_boolean(ConfigFile * cfg, gchar * section, gchar * key, gboolean value)
{
	...
	if (value)
		sven_cfg_write_string(cfg, section, key, "TRUE");
	else
		sven_cfg_write_string(cfg, section, key, "FALSE");
}
...
void sven_cfg_remove_section(ConfigFile * cfg, gchar * section)
{
	ConfigSection *sect;
	ConfigLine *line;
	...
	if ((sect = sven_cfg_find_section(cfg, section)) != NULL)
	{
...
void sven_cfg_remove_key(ConfigFile * cfg, gchar * section, gchar * key)
{
	...
	if ((sect = sven_cfg_find_section(cfg, section)) != NULL)
	{
		if ((line = sven_cfg_find_string(sect, key)) != NULL)
		{
			g_free(line->key);
			g_free(line->value);
			g_free(line);
...
ConfigSection *sven_cfg_find_section(ConfigFile * cfg, char * name)
{
	...
	while (list)
	{
		...
		if (!g_strcasecmp(section->name, name))
			return section;
...
ConfigLine *sven_cfg_find_string(ConfigSection * section, char * key)
{
	...
	while (list)
	{
		line = (ConfigLine *) list->data;
		 ...
		 char *result = g_utf8_normalize (line->key, -1, G_NORMALIZE_ALL);
